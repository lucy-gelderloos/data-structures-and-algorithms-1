...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let names = data.filter(character => character.gender === 'male' || character.gender === 'female').map(character => character.name);
...
let findShortest = (data) => {
  let characters = data.map((character) => {
    return { name: character.name, height: parseInt(character.height), };
  });
