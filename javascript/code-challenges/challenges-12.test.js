...
const validateEmail = (email) => {
  // Solution code here...
  let regExPat = /^[a-zA-Z0-9]+(\.[a-zA-Z0-9]+)?@[a-zA-Z0-9]+\.(com|net|org)$/;
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let regExPat = /^(\(\d{3}\)|\d{3})[- ]?\d{3}[- ]?\d{4}$/;
  return regExPat.test(phoneNumber);
};
