...
const updateNumbers = (obj) => {
  ...
  for (const [name, number] of Object.entries(obj)) {
    formattedEntries.push(`${name}: ${number}`);
  }
...
//   }
// }
const targetCharacter = arr.find(obj => obj.name === character);
if (targetCharacter && targetCharacter.children) {
  return Object.values(targetCharacter.children).length > 0;
}
return false;
};
